...
class Controller extends BaseController
{
    ...
    */
    ...
    //check if the user is not trying to purchase an already purchased plan
    if($user->plan >= $plan->id){
        return redirect()->route('deposits')
      ->with('message', 'You cannot purchase this plan, please refer to your account manager.');
    }
    
    //check if the user is not trying to purchase from his profit
    /*if($user->account_bal - $user->roi < $plan_price){
    ...
    //Check if a user has been barred from making transactions
    if($user->trade_mode == 'off'){
      ...
      if($plan->type=='Main'){
          ...
          users::where('id', $user->id)
          ->update([
         'account_bal'=>$user->account_bal - $plan_price + $plan->gift,
          ...
          foreach($plans as $plan){
              ...
              if($settings->trade_mode=='on'){
                ...
                if($dplan->increment_type=="Percentage"){
                  ...
                  //$increment=(floatval(str_replace(',', '.', str_replace('.', '', $plan->amount)))*$dplan->increment_amount)/100;
                }else{
            ...
            if($condition2){
                 users::where('id', $plan->user)
                    ...
                    'account_bal' => $user->account_bal + floatval(str_replace(',', '.', str_replace('.', '', $plan->amount))),
                ]);
                ...
                //save to transactions history
                    ...
                    $th->plan = $dplan->name;
                    $th->user = $plan->user;
                    $th->amount = floatval(str_replace(',', '.', str_replace('.', '', $plan->amount)));
